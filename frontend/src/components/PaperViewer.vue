    ...
    <el-col>
      <div>
        ...
        <div style="width: 100%; height: 750px; overflow: scroll;" v-show="paperInfoList.includes(localActivePDFName)">
          <div v-show="localActivePDFName == 'PDF'" id="pdf-viewer">
            <embed v-if="pdfBlobUrl" :src="pdfBlobUrl" type="application/pdf" width="100%" height="750px" />
            <p v-else>No PDF selected</p>
          </div>
          <div v-show="localActivePDFName == 'Table'" id="pdf-extracted-table">
            <div v-for="(table, tableIdx) in tableLists" :key="tableIdx" style="text-align: center; padding: 3px;">
              ...
              <div class="demonstration">
                ...
                <el-card v-for="(tablmention, tablmentionIdx) in table.table_mentioned" :key="tablmentionIdx"
                  style="margin: 5px;">
                  <div>
          ...
          <div v-show="localActivePDFName == 'Figure'" id="pdf-extracted-figure">
            <div style="padding: 10px;">
              <div class="block demo-image" v-for="(fig, figIdx) in figLists" :key="figIdx">
                <el-image style="width: 80%" :src="fig.figure_url" :preview-src-list="[fig.figure_url]"></el-image>
                <div class="demonstration">{{ fig.figure_name }}: {{ fig.figure_caption }}</div>
                <div class="demonstration">
                  <el-divider><el-link>Mentions</el-link></el-divider>
                  <el-card v-for="(figmention, figmentionIdx) in fig.figure_mentioned" :key="figmentionIdx"
                    style="margin: 5px;">
                    <div>
          ...
          <div v-show="localActivePDFName == 'Meta'" id="pdf-extracted-meta" style="font-size: 13px; padding: 20px;">
            <el-form size="small" label-position="left" label-width="70px">
              ...
              <el-form-item label="Venue">
                <el-input v-model="localMetaInfo.Journal_or_Conference"
                  @input="updateMetaInfo('Journal_or_Conference', $event)"></el-input>
              </el-form-item>
...
export default {
  ...
  watch: {
    ...
    selectedFile: {
      immediate: true,
      handler(newFile) {
        console.log("selectedFile changed:", newFile);
        if (newFile) {
          this.renderPDF(newFile);
        } else {
          this.pdfBlobUrl = null;  // Clear the URL if no file is selected
        }
      }
    }
  },
  methods: {
    ...
    renderPDF(file) {
      console.log("renderPDF called with file:", file);
      if (file) {
        ...
        reader.onloadend = (e) => {
          ...
          console.log("New pdfBlobUrl created:", this.pdfBlobUrl);
        };
        reader.readAsArrayBuffer(file);
      } else {
        console.log("No file provided to renderPDF");
